...
function calculateHealth(repo) {
    ...
    const sourceFiles = repo.files.filter(f => ['.ts', '.js', '.tsx', '.jsx', '.py', '.cs', '.java', '.go', '.rb'].includes(f.extension));
    ...
    const hasContributing = repo.files.some(f => f.name.toLowerCase() === 'contributing.md' || f.name.toLowerCase() === 'contribute.md');
    checks.push({
    ...
    // 10. Changelog (5 pts)
    const hasChangelog = repo.files.some(f => f.name.toLowerCase() === 'changelog.md' || f.name.toLowerCase() === 'changes.md');
    checks.push({
        name: 'Changelog',
        passed: hasChangelog,
    ...
    const hasEnvExample = repo.files.some(f => f.name === '.env.example' || f.name === '.env.template' || f.name === '.env.sample');
    checks.push({
        name: 'Env template',
    ...
    const hasSetupScript = Object.keys(repo.scripts).some(s => ['dev', 'start', 'serve', 'setup', 'install'].includes(s));
    checks.push({
        name: 'Setup scripts',
...
function generateScoreBar(percentage) {
