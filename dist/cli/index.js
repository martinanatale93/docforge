...
program
    ...
    if ((0, engine_2.detectFolderConflict)(resolvedOutput)) {
        console.log(chalk_1.default.yellow.bold('⚠️  Warning: ') + chalk_1.default.yellow(`The folder "${opts.output}/" already exists and contains files not generated by Docforge.`));
        console.log(chalk_1.default.yellow('   Continuing will overwrite files with matching names in that folder.'));
        console.log('');
        const readline = await Promise.resolve().then(() => __importStar(require('readline')));
        const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
        const answer = await new Promise(resolve => {
            rl.question(chalk_1.default.cyan('   Continue? (y/N) '), resolve);
        });
        rl.close();
        if (answer.toLowerCase() !== 'y') {
            console.log(chalk_1.default.gray('   Aborted. Use --output <other-folder> to write to a different directory.'));
            process.exit(0);
        }
        console.log('');
    }
